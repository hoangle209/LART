...
class lart_transformer(nn.Module):
    def __init__(self, opt, dim, depth, heads, mlp_dim, dim_head = 64, dropout = 0., emb_dropout = 0., droppath = 0., device=None):
        ...
        if("joints_2D" in self.cfg.extra_feat.enable):
            self.joint_encoder       = nn.Sequential(
                                            nn.Linear(self.cfg.extra_feat.joints_2D.dim, self.cfg.extra_feat.joints_2D.mid_dim), nn.ReLU(), 
                                            nn.Linear(self.cfg.extra_feat.joints_2D.mid_dim, self.cfg.extra_feat.joints_2D.mid_dim), nn.ReLU(),         
                                            nn.Linear(self.cfg.extra_feat.joints_2D.mid_dim, self.cfg.extra_feat.joints_2D.en_dim),
                                        )
