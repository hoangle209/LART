...
# from src.ActivityNet.Evaluation.ava import object_detection_evaluation
# from src.ActivityNet.Evaluation.ava import standard_fields
...
#     """
#     excluded = set()
#     with open("/datasets01/AVA/080720/frame_list/ava_val_excluded_timestamps_v2.2.csv", "r") as f:
#         reader = csv.reader(f)
#         for row in reader:
#             assert len(row) == 2, "Expected only 2 columns, got: " + row
#             excluded.add(make_image_key(row[0], row[1]))
#     return excluded
#   exluded_keys = read_exclusions()

#   # Reads the ground truth data.
#   boxes, labels, _, included_keys = read_csv(groundtruth, class_whitelist, 0)
#   start = time.time()
#   for image_key in boxes:
#     if image_key in exluded_keys:
#       logging.info("bad key found!")
#       continue
...
#       # logging.info(("Found detections for image %s which is not part of the the ground truth. They will be ignored."), image_key)
#       continue
#     if image_key in exluded_keys:
#       print("bad key found!")
#       continue
...
#   # import joblib
#   # joblib.dump([metrics, categories, class_whitelist], "_TMP/ava.pkl")

#   return metrics, categories, class_whitelist
...
#   metrics, categories, class_whitelist = run_evaluation(**vars(args))
#   pprint.pprint(metrics, indent=2)
...
from lart.ActivityNet.Evaluation.ava import object_detection_evaluation
from lart.ActivityNet.Evaluation.ava import standard_fields
